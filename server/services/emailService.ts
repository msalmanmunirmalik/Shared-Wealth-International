...
class EmailService {
    ...
    try {
      ...
      templateFiles.forEach(file => {
        if (file.endsWith('.html')) {
          ...
          // Extract subject from HTML comment or use template name
   ...
   */
  private createDefaultTemplates(templatesDir: string): void {
    const templates = {
      'welcome': {
...
© 2024 Shared Wealth International. All rights reserved.`
  ...
  /**
   * Format template name to subject
   */
  private formatSubject(templateName: string): string {
    return templateName
      .split('-')
   ...
   */
  async sendPasswordResetEmail(email: string, name: string, resetUrl: string, expiryHours: number = 24): Promise<boolean> {
    return this.sendTemplateEmail({
      to: email,
      subject: 'Password Reset Request',
   ...
   */
  async sendCompanyApprovalEmail(email: string, name: string, companyName: string): Promise<boolean> {
    return this.sendTemplateEmail({
      to: email,
      subject: 'Company Application Approved',
   ...
   */
  async sendCompanyRejectionEmail(email: string, name: string, companyName: string, reason?: string): Promise<boolean> {
...
© 2024 Shared Wealth International. All rights reserved.
   ...
   */
  async sendNotificationEmail(email: string, subject: string, message: string, actionUrl?: string, actionText?: string): Promise<boolean> {
    return this.sendEmail({
      to: email,
      subject,
      html: `
