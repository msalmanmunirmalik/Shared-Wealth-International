...
export class SharingController {
   ...
   * Share content
   */
  static async shareContent(req: AuthenticatedRequest, res: Response<ApiResponse<ShareData>>) {
    try {
   ...
   */
  static async getShareStats(req: Request, res: Response<ApiResponse<ShareStats>>) {
    try {
      ...
      shares.forEach(share => {
        ...
        // Check if this is the current user's share
  ...
  /**
   * Get users who shared specific content
   */
  static async getContentShares(req: Request, res: Response<ApiResponse<ShareData[]>>) {
    try {
      const { contentId, contentType } = req.params;
      ...
      const shares = await DatabaseService.query(`
        ...
        LIMIT $3 OFFSET $4
      `, [contentId, contentType, parseInt(limit as string), parseInt(offset as string)]);
   ...
   */
  static async getUserShares(req: AuthenticatedRequest, res: Response<ApiResponse<ShareData[]>>) {
    try {
      const userId = req.user?.id;
      const { limit = 50, offset = 0 } = req.query;
   ...
   */
  static async generateShareableLink(req: Request, res: Response<ApiResponse<{ shareableLink: string }>>) {
    try {
      const { contentId, contentType } = req.params;
      const { platform = 'internal' } = req.query;

      // Generate a shareable link based on content type
   ...
   */
  static async getTrendingSharedContent(req: Request, res: Response<ApiResponse<any[]>>) {
    try {
