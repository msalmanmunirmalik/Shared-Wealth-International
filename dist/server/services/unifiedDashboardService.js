...
export class UnifiedDashboardService {
    static async getDashboardData(userId, filters) {
        try {
            const sections = filters.sections?.split(',') || ['user', 'companies', 'analytics', 'notifications', 'activity', 'widgets', 'content', 'social'];
            const [userData, companiesData, analyticsData, notificationsData, activityData, widgetsData, contentData, socialData, adminData] = await Promise.all([
    ...
    static async getUserDashboardData(userId, period) {
        try {
            const [userData, analyticsData, socialData, contentData] = await Promise.all([
    ...
    static async getCompanyDashboardData(companyId, period) {
        try {
            const [companyData, analyticsData, contentData, teamData] = await Promise.all([
    ...
    static async getAdminDashboardData(period) {
        try {
            const [platformStats, userStats, companyStats, contentStats, systemStats] = await Promise.all([
    ...
    static async getDashboardWidgets(userId, dashboardType) {
        try {
    ...
    static async updateDashboardWidgets(userId, widgets) {
        try {
            const upsertQuery = `
    ...
    static async getActivityFeed(userId, filters) {
        try {
            let whereCondition = 'WHERE user_id = $1';
            let queryParams = [userId];
            let paramIndex = 2;
    ...
    static async checkCompanyAccess(userId, companyId) {
        try {
            const query = `
    ...
    static async getRecentContent(userId) {
        try {
            const query = `
        SELECT 
          uc.*,
    ...
    static async getCompanyContent(companyId) {
        try {
            const query = `
        SELECT 
          uc.*,
    ...
    static async getCompanyTeam(companyId) {
        try {
            const query = `
        SELECT 
          u.id,
    ...
    static async getAdminStats(period) {
        try {
            const periodDays = period === '7d' ? 7 : period === '30d' ? 30 : 90;
            const query = `
        SELECT 
    ...
    static async getPlatformStats(period) {
        try {
            const periodDays = period === '7d' ? 7 : period === '30d' ? 30 : 90;
            const query = `
        SELECT 
    ...
    static async getCompanyStats(period) {
        try {
            const periodDays = period === '7d' ? 7 : period === '30d' ? 30 : 90;
            const query = `
        SELECT 
    ...
    static async getContentStats(period) {
        try {
            const periodDays = period === '7d' ? 7 : period === '30d' ? 30 : 90;
            const query = `
        SELECT 
    ...
    static getDefaultWidgets(dashboardType) {
        const baseWidgets = [
            {
                id: 'welcome',
                type: 'welcome',
...
//# sourceMappingURL=unifiedDashboardService.js.map