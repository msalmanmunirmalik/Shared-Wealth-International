...
# DirectAdmin Deployment ZIP Creator with MIME Type Fixes
# Creates a deployment package that fixes MIME type issues

set -e

echo "🚀 Creating DirectAdmin Deployment ZIP with MIME Type Fixes"
echo "=========================================================="
...
# Configuration
ZIP_NAME="shared-wealth-directadmin-$(date +%Y%m%d-%H%M%S).zip"
DEPLOYMENT_DIR="./deployment-zip"
PACKAGE_DIR="${DEPLOYMENT_DIR}/shared-wealth-directadmin"
...
# Create nginx configuration (for nginx servers)
...
Package Version: 2.0.0 (MIME Type Fixes)
Created: $(date)
Domain: sharedwealth.net

Contents:
- Frontend build (React + Vite) with MIME type fixes
...
Files:
- public_html/ (Frontend build with MIME type fixes)
...
cd ${DEPLOYMENT_DIR}
zip -r "${ZIP_NAME}" shared-wealth-directadmin

echo -e "${GREEN}✅ Deployment ZIP created successfully!${NC}"
echo -e "${YELLOW}📁 Package location: ${DEPLOYMENT_DIR}/shared-wealth-directadmin${NC}"
echo -e "${YELLOW}📦 ZIP Archive location: ${DEPLOYMENT_DIR}/${ZIP_NAME}${NC}"
echo ""
echo -e "${BLUE}📋 Package Contents:${NC}"
ls -la shared-wealth-directadmin/
echo ""
echo -e "${GREEN}🚀 Ready for DirectAdmin deployment with MIME type fixes!${NC}"
...
echo ""
echo -e "${GREEN}This package will fix the 'Expected a JavaScript-or-Wasm module script' error!${NC}"
