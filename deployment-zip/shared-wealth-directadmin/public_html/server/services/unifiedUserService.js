...
export class UnifiedUserService {
    static async register(userData) {
        try {
            ...
            const existingUser = await DatabaseService.query('SELECT id FROM users WHERE email = $1', [email]);
    ...
    static async login(email, password) {
      ...
      `;
            ...
            const token = jwt.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || 'fallback-secret', { expiresIn: '7d' });
            await DatabaseService.query('UPDATE users SET last_login = NOW(), updated_at = NOW() WHERE id = $1', [user.id]);
            return {
                success: true,
                data: {
                    user: {
    ...
    static async logout(userId) {
        try {
            await DatabaseService.query('UPDATE users SET last_logout = NOW(), updated_at = NOW() WHERE id = $1', [userId]);
            return {
                success: true,
                data: true
            };
    ...
    static async updateUserProfile(userId, profileData) {
        try {
            ...
            if (profileData.firstName !== undefined)
                fieldsToUpdate.first_name = profileData.firstName;
            if (profileData.lastName !== undefined)
                fieldsToUpdate.last_name = profileData.lastName;
            if (profileData.bio !== undefined)
                fieldsToUpdate.bio = profileData.bio;
            if (profileData.avatar !== undefined)
                fieldsToUpdate.avatar = profileData.avatar;
            if (profileData.preferences !== undefined)
                fieldsToUpdate.preferences = JSON.stringify(profileData.preferences);
    ...
    static async changePassword(userId, currentPassword, newPassword) {
        try {
            const userResult = await DatabaseService.query('SELECT password FROM users WHERE id = $1', [userId]);
            ...
            await DatabaseService.query('UPDATE users SET password = $1, updated_at = NOW() WHERE id = $2', [hashedNewPassword, userId]);
            return {
                success: true,
                data: true
            };
    ...
    static async updateUser(userId, updateData) {
        try {
            ...
            if (updateData.firstName !== undefined)
                fieldsToUpdate.first_name = updateData.firstName;
            if (updateData.lastName !== undefined)
                fieldsToUpdate.last_name = updateData.lastName;
    ...
    static async requestPasswordReset(email) {
        try {
            const userResult = await DatabaseService.query('SELECT id FROM users WHERE email = $1', [email]);
            if (userResult.rows.length === 0) {
                return {
                    success: false,
                    message: 'User not found'
            ...
            await DatabaseService.query('UPDATE users SET password_reset_token = $1, password_reset_expires = $2, updated_at = NOW() WHERE email = $3', [resetToken, resetExpires, email]);
            return {
                success: true,
                data: true
            };
    ...
    static async resetPassword(token, newPassword) {
        try {
            const userResult = await DatabaseService.query('SELECT id FROM users WHERE password_reset_token = $1 AND password_reset_expires > NOW()', [token]);
            if (userResult.rows.length === 0) {
                return {
                    success: false,
            ...
            await DatabaseService.query('UPDATE users SET password = $1, password_reset_token = NULL, password_reset_expires = NULL, updated_at = NOW() WHERE password_reset_token = $2', [hashedPassword, token]);
            return {
                success: true,
                data: true
            };
...
//# sourceMappingURL=unifiedUserService.js.map