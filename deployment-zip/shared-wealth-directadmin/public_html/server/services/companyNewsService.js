...
export class CompanyNewsService {
    static async getAllCompanyNews(limit = 20, offset = 0) {
        try {
            const query = `
        SELECT 
          cn.*,
    ...
    static async getCompanyNews(companyId) {
        try {
            const query = `
        SELECT 
          cn.*,
    ...
    static async createCompanyPost(companyId, userId, postData) {
        try {
            const companyQuery = `
    ...
    static async updateCompanyPost(companyId, postId, userId, updateData) {
        try {
            const verifyQuery = `
    ...
    static async deleteCompanyPost(companyId, postId, userId) {
        try {
            const verifyQuery = `
        SELECT cn.id FROM company_news cn
        JOIN companies c ON cn.company_id = c.id
...
//# sourceMappingURL=companyNewsService.js.map